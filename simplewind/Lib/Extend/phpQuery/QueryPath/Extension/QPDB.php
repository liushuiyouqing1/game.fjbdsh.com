...
class QPDB implements QueryPathExtension
{
	...
	{
		$this->opts = $options + array('username' => NULL, 'password' => NULL, 'db params' => array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION),);
		$this->dsn = $dsn;
		...
		if ($this->cycleRows) {
			while (($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) !== FALSE) {
				foreach ($columns as $col) {
					if (isset($row[$col])) {
						$data = $row[$col];
						if ($hasWrap) $data = qp()->append($wrap)->deepest()->append($data)->top();
						$this->qp->$qpFunc($data);
					}
				}
			}
...
if (!defined('QPDB_OVERRIDE')) QueryPathExtensionRegistry::extend('QPDB');