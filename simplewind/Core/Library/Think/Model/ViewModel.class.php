...
class ViewModel extends Model
{
	...
	{
		if (isset($options['field'])) $options['field'] = $this->checkFields($options['field']); else $options['field'] = $this->checkFields();
				...
				foreach ($fields as $key => $field) {
					if (false !== $_field = array_search($field, $val, true)) {
						if (is_numeric($_field)) {
							...
							if (false !== strpos($_field, '*') || false !== strpos($_field, '(') || false !== strpos($_field, '.')) $array[] = $_field . ' AS ' . $field; else $array[] = $k . '.' . $_field . ' AS ' . $field;
						}
					}
				}
			}
