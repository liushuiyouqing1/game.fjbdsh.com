...
class QiniuStorage
{
	...
	public function upload($config, $file)
	{
		...
		$data = array();
		$fields = array('token' => $uploadToken, 'key' => $config['saveName'] ?: $file['fileName'],);
		...
		switch ($type) {
			case 'img':
				...
				if ($param['imageInfo']) {
					...
					if ($param['w']) $url .= "/w/{$param['w']}";
					if ($param['h']) $url .= "/h/{$param['h']}";
					if ($param['q']) $url .= "/q/{$param['q']}";
					if ($param['format']) $url .= "/format/{$param['format']}";
				}
				break;
			case 'video':
			case 'doc':
				$url = $this->downLink($key);
				$url .= '?md2html';
				if (isset($param['mode'])) $url .= '/' . (int)$param['mode'];
				if ($param['cssurl']) $url .= '/' . self::Qiniu_Encode($param['cssurl']);
				break;
		}
		return $url;
	}
