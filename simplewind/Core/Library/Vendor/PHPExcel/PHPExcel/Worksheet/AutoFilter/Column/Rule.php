...
class PHPExcel_Worksheet_AutoFilter_Column_Rule
{
	...
	private static $_ruleTypes = array(self::AUTOFILTER_RULETYPE_FILTER, self::AUTOFILTER_RULETYPE_DATEGROUP, self::AUTOFILTER_RULETYPE_CUSTOMFILTER, self::AUTOFILTER_RULETYPE_DYNAMICFILTER, self::AUTOFILTER_RULETYPE_TOPTENFILTER,);
	...
	{
		if (empty($pOperator)) $pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL;
	...
	public function setRule($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL, $pValue = '', $pGrouping = NULL)
	{
		...
		if ($pGrouping !== NULL) $this->setGrouping($pGrouping);
		return $this;
	}
