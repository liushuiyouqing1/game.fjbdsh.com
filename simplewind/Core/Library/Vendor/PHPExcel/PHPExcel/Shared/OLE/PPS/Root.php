...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS
{
	...
	public function save($filename)
	{
		...
		if (is_resource($filename)) {
			...
			if ($this->_tmp_dir === NULL) $this->_tmp_dir = PHPExcel_Shared_File::sys_get_temp_dir();
	...
	public function _saveHeader($iSBDcnt, $iBBcnt, $iPPScnt)
	{
		...
		fwrite($FILE, "\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" . "\x00\x00\x00\x00" . "\x00\x00\x00\x00" . "\x00\x00\x00\x00" . "\x00\x00\x00\x00" . pack("v", 0x3b) . pack("v", 0x03) . pack("v", -2) . pack("v", 9) . pack("v", 6) . pack("v", 0) . "\x00\x00\x00\x00" . "\x00\x00\x00\x00" . pack("V", $iBdCnt) . pack("V", $iBBcnt + $iSBDcnt) . pack("V", 0) . pack("V", 0x1000) . pack("V", $iSBDcnt ? 0 : -2) . pack("V", $iSBDcnt));
		if ($iBdCnt < $i1stBdL) {
