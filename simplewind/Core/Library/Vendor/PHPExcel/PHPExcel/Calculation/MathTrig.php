...
class PHPExcel_Calculation_MathTrig
{
	...
	public static function LOG_BASE($number = NULL, $base = 10)
	{
		...
		if ((!is_numeric($base)) || (!is_numeric($number))) return PHPExcel_Calculation_Functions::VALUE();
		if (($base <= 0) || ($number <= 0)) return PHPExcel_Calculation_Functions::NaN();
	...
	{
		$number = PHPExcel_Calculation_Functions::flattenSingleValue($number);
		if (is_bool($number)) return (int)$number;
		if (is_numeric($number)) {
			if ($number == 0.0) {
				return 0;
			}
	...
	public static function TRUNC($value = 0, $digits = 0)
	{
		...
		$digits = PHPExcel_Calculation_Functions::flattenSingleValue($digits);
		if ((!is_numeric($value)) || (!is_numeric($digits))) return PHPExcel_Calculation_Functions::VALUE();
		$digits = floor($digits);
		$adjust = pow(10, $digits);
		if (($digits > 0) && (rtrim(intval((abs($value) - abs(intval($value))) * $adjust), '0') < $adjust / 10)) return $value;
