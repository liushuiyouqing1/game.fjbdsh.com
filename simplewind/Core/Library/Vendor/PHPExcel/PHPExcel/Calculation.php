...
class PHPExcel_Calculation
{
	...
	public function setLocale($locale = 'en_us')
	{
		...
		if (count(self::$_validLocaleLanguages) == 1) self::_loadLocales();
	...
	private function _parseFormula($formula, PHPExcel_Cell $pCell = NULL)
	{
		...
		while (TRUE) {
			...
			if ($opCharacter == '-' && !$expectingOperator) {
				...
				while (($o2 = $stack->pop()) && $o2['value'] != '(') {
					if ($o2 === NULL) return $this->_raiseFormulaError('Formula Error: Unexpected closing brace ")"'); else $output[] = $o2;
				}
				$d = $stack->last(2);
				...
				while (($o2 = $stack->pop()) && $o2['value'] != '(') {
					if ($o2 === NULL) return $this->_raiseFormulaError("Formula Error: Unexpected ,"); else $output[] = $o2;
				}
		...
		while (($op = $stack->pop()) !== NULL) {
			if ((is_array($op) && $op['value'] == '(') || ($op === '(')) return $this->_raiseFormulaError("Formula Error: Expecting ')'");
			$output[] = $op;
		}
		return $output;
	}
