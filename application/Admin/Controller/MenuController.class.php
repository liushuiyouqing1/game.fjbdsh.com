			...
			if ($this->menu_model->create() !== false) {
				if ($this->menu_model->add() !== false) {
					...
					if ($find_rule_count == 0) {
						$this->auth_rule_model->add(array("name" => $name, "module" => $app, "type" => "admin_url", "title" => $menu_name));
					}
			...
			if ($this->menu_model->create() !== false) {
				if ($this->menu_model->save() !== false) {
					...
					if ($find_rule_count == 0) {
						...
						$this->auth_rule_model->where($mwhere)->save(array("name" => $name, "module" => $app, "type" => "admin_url", "title" => $menu_name));
					}
					$this->_export_app_menu_default_lang($app);
					$this->success("更新成功！");
				} else {
	...
	private function _import_menu($menus, $parentid = 0, &$error_menus = array())
	{
		foreach ($menus as $menu) {
			...
			if (!$find_rule) {
				...
				$this->auth_rule_model->where($mwhere)->save(array("module" => $app, "type" => "admin_url", "title" => $menu['name']));
			}
							...
							if (!(strpos($mf, ".") === 0)) {
								...
								foreach ($methods as $a) {
									if (!(strpos($a, "_") === 0) && !(strpos($a, "spmy_") === 0)) {
										...
										$find_rule = $this->auth_rule_model->where($mwhere)->find();
										if (!$find_rule) {
											$this->auth_rule_model->add(array("name" => $name, "module" => $g, "type" => "admin_url", "title" => ""));
										}
									}
								}
							}
