...
class AdminYouxiController extends AdminbaseController
{
	...
	public function GameList()
	{
		...
		$posts = M('game')->where($where)->order(array("`sort`" => "desc"))->limit($page->firstRow, $page->listRows)->select();
	...
	public function RuleList()
	{
		...
		$page = $this->page($count, 10);
		$posts = M('rule')->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
		$this->assign('posts', $posts);
		$this->assign('page', $page->show('Admin'));
		$game = M('game')->select();
