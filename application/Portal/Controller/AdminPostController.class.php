...
class AdminPostController extends AdminbaseController
{
	...
	private function _lists($where = array())
	{
		...
		if (!empty($start_time)) {
			$where['post_date'] = array(array('EGT', $start_time));
		}
		...
		$this->posts_model->alias("a")->where($where);
		if (!empty($term_id)) {
			$this->posts_model->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id");
		}
		$count = $this->posts_model->count();
		$page = $this->page($count, 20);
		$this->posts_model->alias("a")->join("__USERS__ c ON a.post_author = c.id")->where($where)->limit($page->firstRow, $page->listRows)->order("a.post_date DESC");
		if (empty($term_id)) {
