...
class UserController extends HomebaseController
{
  ...
  <div class="gerenzx-fjh gerenzx-fjh-color">' . rommidxx($value['room']) . '</div> 
	...
	public function shengchengfangka()
	{
		$post = I('post.');
	...
	public function editPasswordPost()
	{
		...
		if ($old_user_pass && $old_user_two_pass) {
		...
		} elseif ($old_user_pass && !$old_user_two_pass) {
			if (md5($old_user_pass) != $this->user['user_pass']) {
				$this->msg('原密码错误', $url);
			}
			$data['user_pass'] = md5($post['new_user_pass']);
	...
	public function cheackOrder()
	{
		...
		foreach ($provide_order as $key => $value) {
			...
			if ($diffHours > $pay_time) {
				$deduct_parent_money = $this->bonus['deduct_parent_money'];
		...
		$income_time = $this->bonus['income_time'];
	...
	public function myTerm()
	{
		...
		if ($child) {
			...
			$provide_help_data = $this->provide_help->where($where)->order(array("id" => "desc"))->limit($provide_help->firstRow, $provide_help->listRows)->select();
			...
			$get_help_data = $this->get_help->where($where)->order(array("id" => "desc"))->limit($get_help->firstRow, $get_help->listRows)->select();
	...
	public function activationCode()
	{
		...
		$record = $this->activation_code->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
	...
	public function pdb()
	{
		...
		$page = $this->page($count, 20);
		$record = $this->pdb->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
		$yes['user_login'] = $this->user_login;
		$yes['status'] = 0;
	...
	public function turntable()
	{
		...
		$page = $this->page($count, 20);
		$record = $award->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
		$this->assign('record', $record);
		$this->assign("page", $page->show('Admin'));
		$this->display();
	}
	...
	public function getPrizeWinner()
	{
		...
		$prize_arr = array('0' => array('id' => 1, 'min' => 1, 'max' => 29, 'prize' => '一等奖《苹果7》', 'v' => 0.1), '1' => array('id' => 2, 'min' => 302, 'max' => 328, 'prize' => '二等奖《三星 Galaxy S7》', 'v' => 0.3), '2' => array('id' => 3, 'min' => 242, 'max' => 268, 'prize' => '三等奖《苹果 iPad mini 4》', 'v' => 0.6), '3' => array('id' => 4, 'min' => 182, 'max' => 208, 'prize' => '四等奖《意式咖啡机》', 'v' => 3), '4' => array('id' => 5, 'min' => 122, 'max' => 148, 'prize' => '五等奖《飞利浦音响》', 'v' => 7), '5' => array('id' => 6, 'min' => 62, 'max' => 88, 'prize' => '六等奖《充电宝》', 'v' => 10), '6' => array('id' => 7, 'min' => array(32, 92, 152, 212, 272, 332), 'max' => array(58, 118, 178, 238, 298, 358), 'prize' => '《感谢参与》', 'v' => 79));
