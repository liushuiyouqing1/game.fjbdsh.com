...
class HomeController extends HomebaseController
{
  ...
  <div class="gerenzx-fjh gerenzx-fjh-color">' . rommidxx($value['id']) . '</div> 
	...
	public function index()
	{
		...
		$provide_help = $this->provide_help->where($p_where)->order(array("id" => "desc"))->limit($provide_page->firstRow, $provide_page->listRows)->select();
		$status[0] = '等待中';
		$status[1] = '已匹配';
		...
		$get_help = $this->get_help->where($g_where)->order(array("id" => "desc"))->limit($get_page->firstRow, $get_page->listRows)->select();
		$status[0] = '等待中';
		$status[1] = '已匹配';
		$confirm_status[0] = "未确认";
		$confirm_status[1] = "已确认";
		...
		$match = $this->match->where($m_where)->order(array("id" => "desc"))->limit($match_page->firstRow, $match_page->listRows)->select();
		$p_status[0] = '待付款';
		$p_status[1] = '已付款';
		$p_status[2] = '交易完成';
	...
	public function news()
	{
		...
		$res = $posts_model->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
	...
	public function linkUs()
	{
		...
		$page = $this->page($count, 20);
		$res = $guest_book->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
