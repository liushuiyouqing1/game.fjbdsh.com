...
class DateController extends HomebaseController
{
	function kj()
	{
		...
		eval("\$now=\$this->" . $typelx['appear'] . "(\$typelx[data]);");
		...
		foreach ($planlist as $key => $value) {
			...
			if ($plan_data) {
				eval("\$bjdata=\$this->" . $location['appear'] . "(\$data[data],\$value[weizhi]);");
				eval("\$zt=\$this->" . $sort['appear'] . "(\$bjdata,\$plan_data[data],\$sort[method]);");
				if ($zt == 1) {
		...
		foreach ($plan_data as $key => $value) {
			...
			eval("\$bjdata=\$this->" . $location['appear'] . "(\$data[data],\$value[weizhi]);");
			eval("\$zt=\$this->" . $sort['appear'] . "(\$bjdata,\$value[data],\$sort[method]);");
			$bz = $value['bz'] + 1;
			$data['zt'] = $zt;
